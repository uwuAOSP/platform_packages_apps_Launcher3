...
class LauncherSmartspacerView @JvmOverloads constructor(
    ...
    private inner class LauncherPopupFactory : PopupFactory {
        override fun createPopup(
            ...
            val position = Rect()
            launcher.dragLayer.getDescendantRectRelativeToSelf(anchorView, position)
            ...
            ).ifEmpty { listOf(createSettingsFallbackOption(context)) }
            val popup = OptionsPopupView.show<Launcher>(launcher, RectF(position), options, true)
            return object : Popup {
